...
const groupLiterals = (...literals) => {
	...
	for ( const literal of literals ) {
		if ( typeof literal === 'string' || literal instanceof String ) {
			...
			uniqueStrings.set(key, {key, literal}); // eslint-disable-line object-property-newline
		} else if ( literal instanceof RegExp ) {
			const key = literal.source;
			uniqueRegExps.set(key, {key, literal}); // eslint-disable-line object-property-newline
		} else if ( literal instanceof PrefixLiteral ) {
			const key = literal.key();
			uniquePrefixes.set(key, {key, literal}); // eslint-disable-line object-property-newline
		} else if ( literal instanceof AllLiteral ) {
			const key = literal.key();
			uniqueAlls.set(key, {key, literal}); // eslint-disable-line object-property-newline
		} else {
